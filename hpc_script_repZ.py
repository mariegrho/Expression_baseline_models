...
from model.models import Repression_Z
...
@click.option("--n_cores",      type=int, default=None,     help="The number of cores to be used for multiprocessing")
@click.option("--eval",         is_flag=True,               help="Run evaluation of mean posterior parameters")
@click.option("--kernel",       type=str, default="svi",    help="Inference kernel to use: svi or nuts")
@click.option("--t_end",       type=int, default=120,     help="end point of simulation: 120 or 24 hpf")
@click.option("--seed",     type=int, default=1,        help="simulation seed")
def main(gene_id, n_cores, eval, kernel, t_end, seed):
    ...
    if n_cores is not None: sim.config.multiprocessing.cores = n_cores
    ...
    mod.state_variables["M"]["y0"] = sim.observations.y[0].item()
