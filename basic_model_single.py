...
def prepare_dataset(gene_id, dataset, t_end):

    try:
        transcript_data = {
            "White": xr.load_dataset("data/genes_tpms_white_pauli_JN_BK_mean.nc").sel(source="White", drop=False).sel(time=slice(0, t_end)),
            "Pauli": xr.load_dataset("data/genes_tpms_white_pauli_JN_BK_mean.nc").sel(source="Pauli", drop=False).sel(time=slice(0, t_end)),
            "JN": xr.load_dataset("data/genes_tpms_white_pauli_JN_BK_mean.nc").sel(source="JN", drop=False).sel(time=slice(0, t_end)),
            "BK": xr.load_dataset("data/genes_tpms_white_pauli_JN_BK_mean.nc").sel(source="BK", drop=False).sel(time=slice(0, t_end)),
            "Medina_Munoz_polyA": xr.load_dataset("data/dataset_medina_munoz.nc").sel(selection_method="polyA+", drop=True), 
            "Medina_Munoz_ribo": xr.load_dataset("data/dataset_medina_munoz.nc").sel(selection_method="ribo-", drop=True), 
        }[dataset]
    except KeyError:
        raise ValueError(f"Dataset {dataset} not found. \nChoose from: White, Pauli, JN, BK, Medina_Munoz_polyA, Medina_Munoz_ribo.") 

    try:
        data_g = transcript_data.sel(ensembl_gene_id=gene_id)
        non_nan_times = data_g.tpm.time.values[~np.isnan(data_g.tpm.values)]
        obs = data_g.tpm.sel(time=non_nan_times).to_dataset(name="y") 
...
def main(gene_id, dataset, kernel="nuts", t_end=120, plot=True, smooth=False, skip_duplicates=True, seed=1):
    ...
    obs = prepare_dataset(gene_id, dataset, t_end)
