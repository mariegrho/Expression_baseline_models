...
def plot_pass_rates(df: pd.DataFrame,
                     criteria: list = ("converged", "good_fit", "reliable_loo",
                                        "well_calibrated"),
                     labels: dict = None, hue: str = None, ax=None):
    """
    ...
    if hue is not None:
        for c in criteria:
            for h_val, sub in df.groupby(hue):
                valid = sub[c].dropna()
                n_valid = len(valid)
                pass_pct = valid.mean() * 100 if n_valid > 0 else np.nan
    ...
    if ax is None:
        fig, ax = plt.subplots(figsize=(7.5, 0.5 * len(criteria) * max(n_hue_levels, 1) + 1.5))

    n_total = len(df)/n_hue_levels if hue is not None else len(df)
    if hue is not None:
        sns.barplot(data=plot_df, y="criterion", x="pass_pct", hue=hue, palette=mod_color_dict,
                    order=order, ax=ax, orient="h")
    ...
    ax.set_xlim(0, 100)
    ax.set_xlabel("% of genes passing")
    ax.set_ylabel("")
    ax.set_title(f"Fit-quality criteria pass rates  (n = {int(n_total):,} genes)")
    ax.spines[["top", "right"]].set_visible(False)
    return ax


def plot_status_breakdown(df: pd.DataFrame, status_col: str = "status",
                           hue: str = None, ax=None, palette: dict = None):
    """
    ...
    if hue is not None:
        ...
        if ax is None:
            ...
            fig, ax = plt.subplots(figsize=(7.5, 0.5 * len(order) * n_hue_levels + 1.5) )

        sns.barplot(data=prop_df, y=status_col, x="proportion", hue=hue, palette=mod_color_dict, order=order, ax=ax, orient="h")
        for container in ax.containers:
            ax.bar_label(container, labels=[f"{v.get_width()*100:.1f}%" for v in container], padding=3, fontsize=8)
        ax.legend(title=hue.replace("_", " ").title(), bbox_to_anchor=(1.02, 1), loc="upper left", frameon=False)
        ax.set_xlabel("Proportion of genes (within model variant)")
        ax.set_xlim(0, 1.0)
        ax.set_title("Status breakdown by model variant")
