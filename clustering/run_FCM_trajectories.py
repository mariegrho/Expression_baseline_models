...
# reorder _membership
membership120 = membership120.isel(cluster=new_order).assign_coords(cluster=np.arange(len(new_order)))
membership120.to_netcdf(f"results/{source}_superclusters_membership.nc")

unique, counts = np.unique(labels120.values, return_counts=True)
