...
from model.initialise_single import *
...
def prepare_dataset(gene_id, dataset, model_version, t_end):
    ...
    try:
        transcript_data = {
            "White": xr.load_dataset("data/genes_tpms_white_pauli_JN_BK_mean.nc").sel(source="White").sel(time=slice(0, t_end)),
            "Pauli": xr.load_dataset("data/genes_tpms_white_pauli_JN_BK_mean.nc").sel(source="Pauli").sel(time=slice(0, t_end)),
...
def gof_evaluation(idata, gene_id, model, dataset, out_path):
    ...
    non_nan_times = idata.observed_data.y.time.values[np.isfinite(idata.observed_data.y.values)]
